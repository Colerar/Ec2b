...
static const byte shift_rows_table[] = {
    0, 5, 10, 15, 4, 9, 14, 3, 8, 13, 2, 7, 12, 1, 6, 11
};
static const byte shift_rows_table_inv[] = {
...
static void xorr(byte * a, const byte *b, int n) {
	int i;
	for (i = 0; i < n; i++) {
		a[i] ^= b[i];
	}
...
void oqs_aes128_load_schedule_c(const uint8_t *key, void **_schedule) {
	*_schedule = malloc(16 * 11);
	...
	while (bytes < 176) {                   // Until we have 176 bytes of expanded key, we do the following:
		...
		xorr(t, schedule + bytes - 16, 4);   // We exclusive-or t with the four-byte block 16 bytes before the new expanded key.
		...
		for (j = 0; j < 3; j++) {
			...
			xorr(t, schedule + bytes - 16, 4);   // We exclusive-or t with the four-byte block n bytes before
			memcpy(schedule + bytes, t, 4);     // This becomes the next 4 bytes in the expanded key
			bytes += 4;                         // Keep track of how many expanded key bytes we've added
		}
	}
